...
std::string Opengl::menu(std::vector<std::string> list_games)
{
    int idx = 0;
    ...
    _window.create(sf::VideoMode(600, 400), "OpenGL", sf::Style::Titlebar | sf::Style::Close);
    _window.setFramerateLimit(8);
...
{
    _window.create(sf::VideoMode(_tileSize * 90, _tileSize * 35), "OpenGL", sf::Style::Titlebar | sf::Style::Close);
//    _window.setFramerateLimit(8);
}

void Opengl::init_map()
{
    _mapStartX = 1;
    _mapStartY = 2;
}

void Opengl::getevent(Event &evt)
{
...
sf::Sprite Opengl::set_sprite(int x, int y, std::string src_path)
{
...
int Opengl::getScore() const
{
    return (_score);
}

void Opengl::create_map(std::vector<std::string> src_map)
{
}

void Opengl::move(Event &evt)
{
    _map_carac = _player.back()->move(evt, _map_carac, _bonus);
    text.setString(std::to_string(_player.front()->getScore()));
}

void Opengl::set_map(std::vector<IObject *> &map)
{
...
void Opengl::set_player(std::vector<IEntity *> &player)
{
...
void Opengl::set_enemy(std::vector<IEntity *> &enemies)
{
...
void Opengl::set_bonus(std::vector<IObject *> &bonus)
{
